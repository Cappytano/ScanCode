(function(){
  ...
  function updateWeightPill(){var mode=state.weightSource||'ocr'; if(mode==='ocr'){setPill(pillOCR, state.ocrReady?'Weight: OCR Ready':'Weight: OCR', state.ocrReady?'ok':'warn');}
    ...
    else setPill(pillOCR,'Weight: —','bad');}
  ...
  // ZXingWASM-only
  var haveZX = (typeof window.ZXingWASM !== 'undefined');
  function updateEngineAvailability(){ haveZX = (typeof window.ZXingWASM !== 'undefined'); }
  ...
  function tryZX(polys, cb){
    ...
    if (window.ZXingWASM && ZXingWASM.readBarcodes) {
      ZXingWASM.readBarcodes(id, { tryHarder: true, maxNumberOfSymbols: 1 }).then(function(res){
        if (res && res.length) {
          var r = res[0];
          if (r && r.position) {
            var poly = [r.position.topLeft, r.position.topRight, r.position.bottomRight, r.position.bottomLeft]
              .map(function(p){ return { x: p.x / id.width, y: p.y / id.height }; });
            polys.push(poly);
          }
          cb({ text: r.text, format: r.format || 'unknown', polygon: polys[0] || null, source: 'camera' });
        } else {
          cb(null);
        }
  ...
  btnSnapshot.addEventListener('click', function(){ if(state.autolog){ toast('Turn OFF Auto-logging to use Snapshot.'); return; } var id=frameToImageData(1024); if(!id){ toast('No frame.'); return; }
    function afterNoCode(){ capturePhotoAndOCR(null, function(weight, photoData){ if(weight!=null){ var now=new Date(); var row={id:state.nextRowId++, value:'', format:'', engine:'—', source:'snapshot', date:now.toLocaleDateString(), time:now.toLocaleTimeString(), weight_g:weight, photo:photoData, count:1, notes:noteInput.value||''}; state.rows.push(row); saveData(); renderRows(); toast('Snapshot logged (weight only).'); } else { toast('No code detected.'); } }); }
    if (window.ZXingWASM && ZXingWASM.readBarcodes){
      ZXingWASM.readBarcodes(id, { tryHarder:true, maxNumberOfSymbols:1 }).then(function(res){
        if(res && res.length){ logFromSnapshot({ text: res[0].text, format: (res[0].format || 'unknown') }); }
        else afterNoCode();
      })['catch'](function(){ afterNoCode(); });
    } else afterNoCode();
  });
  ...
  function parseBLEWeightToGrams(dv){ if(!dv||dv.byteLength<3) return null; var flags=dv.getUint8(0); var unitKg=((flags&0x01)===0); var raw=dv.getUint16(1,true); var val=sfloatToNumber(raw); if(val==null) return null; var kg=unitKg?val:(val*0.45359237); return Math.round(kg*1000*100)/100; }
  ...
  function runEngineDiagnostics(){ var lines=[]; lines.push('=== Scan Engine (ZXing WASM only) ==='); lines.push('ZXingWASM present: '+(window.ZXingWASM?'yes':'no')); lines.push('Camera running: '+(state.scanning?'yes':'no')); lines.push('Video dims: '+(video.videoWidth||0)+'x'+(video.videoHeight||0)); alert(lines.join('\\n')); }
